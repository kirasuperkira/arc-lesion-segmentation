-- =============================================================
-- SQL lab — ARC Lesion Segmentation database
-- Run after db/schema.sql and db/seed.py have been applied.
-- Each query is self-contained and can be run independently.
-- =============================================================

-- 1. SELECT + filtering
-- All participants with a Dice Score above 0.3, best first.
SELECT subject_code, dice_score, processed_at
FROM participants
WHERE dice_score > 0.3
ORDER BY dice_score DESC;

-- 2. SELECT + filtering on a range (BETWEEN)
-- Participants with moderate segmentation quality only.
...
-- 3. JOIN
-- Participants together with the algorithm version and run status
-- that produced their result.
SELECT
    p.subject_code,
    p.dice_score,
    r.algorithm_version,
...
-- Average / min / max Dice Score and participant count per algorithm version.
SELECT
    r.algorithm_version,
...
-- 5. GROUP BY + HAVING
-- Algorithm versions whose average Dice Score is below 0.25
...
-- 6. Aggregation with derived column
-- Volume overestimation (%) of the automatic mask vs. ground truth,
-- for participants with a non-zero ground-truth volume.
SELECT
    subject_code,
    lesion_volume_gt,
    lesion_volume_auto,
